...
+ std_dev ^2 = variance = Sum ( Xi - global_mean )^2/ N -1  where N = total_count
...
### Task Controllers
  The FL server controller is made of many task Controllers. 
+ ClientStatsController
+ GlobalVarianceController
+ GlobalMedianController
+ GlobalHistogramController
+ few more todo for categorical stats

#### Overall Control Flow

```mermaid
graph TB;
   A[ClientStatsController] --> B[GlobalVarianceController]; 
   B --> C[GlobalMedianController];
   C --> D[GlobalHistogramController];
   D --> E[Generate Facets-Overview Protobuf]
```
#### Specific Task Controllers
...
### Statistics assemble and Visualization

Once we generated the protobuf defined by Google's Facets-Overview, we can serialize
the results into string and save to a file. 

the result can be then be loaded via HTML via Javascripts or Python in Jupyter notebook

#### **JavaScripts**

For Java scripts demo, one can do the followings
...
<script>
  ...
  function setupVis() {
    ...
    function protoAjax() {
     var jqxhr = $.get( {url: "../data/fed_stats.pb"})
        .done(function(data) {
...
first 
```bash
  cp <saved_result file> to demo/data/fed_stats.pb    
```
...
```
then go to brows browser http://localhost:8000/javascripts/
![img.png](img.png)
